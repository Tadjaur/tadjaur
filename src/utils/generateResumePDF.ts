...
import { getSkillCategories, portfolioData } from "../data/portfolioData";
...
export const generateResumePDF = () => {
	...
	Object.entries(skillCategories).forEach(([category, categorySkills]) => {
		...
		const skillsList = categorySkills
			.map((s) => `${s.name} (${s.years}yr${s.years > 1 ? "s" : ""})`)
			.join(", ");
	...
	education.forEach((edu) => {
		...
		doc.text(
			...
			leftMargin + doc.getTextWidth(edu.institution),
			yPosition,
		);
		doc.setTextColor(...textColor);
		yPosition += 8;
	...
	for (let i = 1; i <= pageCount; i++) {
		...
		doc.text(`Generated from portfolio: ${personalInfo.portfolio}`, 105, 294, {
			align: "center",
		});
	}
